...
export class EditPatientModalComponent {
  ...
  open(patient: Patient) {
    this.formsData = JSON.parse(JSON.stringify(patient));
    this.editModal = this.modalService.open(this.content, { centered: true, backdrop: 'static' });
  }

  close() {
    this.editModal.close('Close click');
  }
